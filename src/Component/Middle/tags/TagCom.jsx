    ...
    const { data: tags = [], isLoading: tagsLoading } = useQuery({
        ...
        queryFn: async () => {
            ...
            if (tagsData.length > 0) {
                if (typeof tagsData[0] === 'object' && tagsData[0].value) {
                    // If it's an array of objects like [{value: 'tag1', label: 'Tag 1'}]
                    return tagsData.map(tag => tag.value);
                } else if (typeof tagsData[0] === 'object' && tagsData[0].label) {
                    // If it's an array of objects like [{label: 'Tag 1'}]
                    return tagsData.map(tag => tag.label);
                }
            }
            ...
            <div className="max-w-7xl mx-auto px-4 py-6">
                ...
                {tagsLoading ? (
                    ...
                    <div className="flex flex-wrap justify-center gap-3">
                        {tags.map((tag) => {
                            // Extract the tag string from object if needed
                            const tagString = typeof tag === 'string' ? tag :
                                tag?.value || tag?.label || 'Unknown Tag';

                            return (
                                <button
                                    key={tagString}
                                    onClick={() => handleTagClick(tagString)}
            ...
            <div className="max-w-7xl mx-auto px-4 pb-8">
                ...
                <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-6">
                    {searchResults.map((post) => (
                        <div
                            key={post._id}
                            className="relative bg-gradient-to-br from-white to-gray-50 rounded-2xl shadow-sm overflow-hidden transform hover:scale-105 hover:shadow-2xl transition-all duration-500 border border-purple-100/50 post-card"
                        >
                            {post.postPhoto && (
                                <div className="relative">
                                    <img
                                        src={post.postPhoto}
                                        alt={post.postTitle}
                                        className="w-full h-25 object-cover transition-transform duration-500 hover:scale-110"
                            ...
                            <div className="p-6">
                                <h3 className="text-sm font-semibold text-gray-900 mb-3 line-clamp-2 leading-tight tracking-tight">
                                    {post.postTitle}
                                </h3>
                                ...
                                <div className="flex justify-between items-center">
                                    {/* <p className="text-sm text-gray-500 italic ">By {post.authorName}</p> */}
                                    <button
                ...
                <div className="flex justify-between items-center mb-6">
                    ...
                    <div className="flex items-center gap-3">
                        <button
                            ...
                            className={`p-2 rounded-full text-white font-semibold transition-all duration-300 shadow-md ${startIndex === 0 ? 'bg-gray-400 cursor-not-allowed' : 'bg-indigo-600 hover:bg-indigo-700'
                                }`}
                        >
                            <FaArrowLeft />
                        </button>
                        <button
                            ...
                            className={`p-2 rounded-full text-white font-semibold transition-all duration-300 shadow-md ${startIndex + postsPerPage >= allPosts.length ? 'bg-gray-400 cursor-not-allowed' : 'bg-indigo-600 hover:bg-indigo-700'
                                }`}
                        >
                            <FaArrowRight />
                        </button>
                    ...
                    <div className="flex flex-col gap-6">
                                        ...
                                        {replyToCommentId === comment._id && (
                                            <div className="mt-2 flex items-center gap-2 ml-4">
                                                ...
                                                <button
                                                    ...
                                                    className={`p-2 rounded-lg transition-colors duration-300 ${replyText.trim() ? 'bg-blue-500 text-white hover:bg-blue-600' : 'bg-gray-300 text-gray-500'
                                                        }`}
                        ...
                        <div className="flex items-center gap-2">
                            ...
                            <button
                                ...
                                className={`p-3 rounded-lg transition-colors duration-300 ${commentText.trim() && user ? 'bg-blue-500 text-white hover:bg-blue-600' : 'bg-gray-300 text-gray-500'
                                    }`}
